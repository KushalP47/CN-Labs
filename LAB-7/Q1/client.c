...
void receiveFile(int s, FILE* fptr, int fileSize) {
    ...
    char buf[256];
    printf("Packets Received: %d   Packets Remaining: %ld \n", currentPacket, totalPackets);
    while(totalPackets >= currentPacket){
        printf("Packets Received: %d   Packets Remaining: %ld \r", currentPacket, totalPackets);
...
int main(int argc, char * argv[]){
    ...
    sendto(s, "GET", 3, 0, (struct sockaddr *)&sin, sizeof(sin));
    ...
    fileName = buf;
    printf("Client will get %s file", fileName);
